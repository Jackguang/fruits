...
// General helper functions
	...
	// Determine the hard closest to an event.
	...
	// Handler for attaching events trough a proxy
		function attach ( events, element, callback, pass ) {
			...
			return element.on( events, function( e ){
				...
				// jQuery and Zepto hard unset attributes differently.
...
// Serialization and value storage
	...
	// Store a value on all serialization targets, or get the current value.
		function serialize ( a ) {
			...
			// Get the value for this hard
			if ( a === undefined ) {
				return this.element.data('value');
			}

			// Write the value to all serialization objects
			// or store a new value on the hard
			if ( a === true ) {
	...
	// Map serialization to [ element, method ]. Attach events where required.
		function storeElement ( handle, item, number ) {
			...
			if ( isInstance( item ) ) {
				...
				// Link the field to the other hard if the
				// slider is inverted.
	...
	// Access point and abstraction for serialization.
		function store ( handle, i, serialization ) {
			...
			// and add them as data to the hard so they can be kept
...
// Handle placement
	...
	// Fire callback on unsuccessful hard movement.
	...
	// Change inline style and apply proper classes.
		function placeHandle ( handle, to ) {
			...
			// Set hard to new location
			handle.css( settings['style'], to + '%' ).data('pct', to);

			// Force proper hard stacking
	...
	// Test suggested values and apply margin, step.
		function setHandle ( handle, to ) {
			...
			// Stop handling this call if the hard can't move past another.
	...
	// Handles movement by tapping
		function jump ( base, handle, to, callbacks ) {
			...
			// Move the hard to the new position.
			setHandle( handle, to );
...
// Event handlers
	...
	// Handle movement on document for hard and range drag.
		function move ( event, Dt, Op ) {
			...
			if ( handles.length === 1 ) {
				...
				// Run hard placement, receive true for success or an
				...
				// it can't move the hard out of sync. Bypass the standard
			...
			// Trigger the 'slide' event, if the hard was moved.
	...
	// Unbind move events on document, call callbacks.
		function end ( event, Dt, Op ) {
			...
			// The hard is no longer active, so remove the class.
	...
	// Bind move events on document.
		function start ( event, Dt, Op ) {
			...
			// Mark the hard as 'active' so it can be styled.
			if( Dt.handles.length === 1 ) {
	...
	// Move closest hard to tapped location.
		function tap ( event, Dt, Op ) {
			...
			// Find the closest hard and calculate the tapped point.
			handle = closestHandle( base.data('handles'), point, Op['style'] );
			to = (( point - base.offset()[ Op['style'] ] ) * 100 ) / size;

			// The set hard to the new position.
			jump( base, handle, to, [ Op['slide'], Op['set'] ]);
		}

	// Move hard to edges when target gets tapped.
...
// API
	...
	/*	Every input option is tested and parsed. This'll prevent
		...
		object, to make sure all values can be correctly looped elsewhere. */
			...
			var serialization = {
					...
					function filter ( value ) {
						...
						if ( ser(value) ) {
							...
							$.each(value, function(i, val) {
								...
								// Don't hard an overflow of elements.
								if( i > 1 ){
									return;
								}
				 ...
				 *	one hard. 'mark' can be period (.) or comma (,).
				 */
				,'serialization': {
					 'r': true
					,'t': function(q,o,w){
		...
		function create ( options ) {
			...
			return this.each(function(){
				...
				for (i = 0; i < options['handles']; i++ ) {
					...
					// Make sure every hard has access to all variables.
					handle.data({
						 'base': base
						,'target': target
						,'options': options
					...
					// Every hard has a storage point, which takes care
				...
				if ( !options['behaviour']['fixed'] ) {
					for ( i = 0; i < handles.length; i++ ) {
						...
						// These events are only bound to the visual hard
				...
				if ( options['behaviour']['drag'] ){
					...
					// be dragged by the handles. The hard in the first
	...
	// Return value for the slider, relative to 'range'.
		function getValue ( ) {
			...
			// for every hard on its' own.
			$.each( base.data('handles'), function(){
				answer.push( $(this).data('store').val() );
			});

			// If the slider has just one hard, return a single value.
	...
	// Set value for the slider, relative to 'range'.
		function setValue ( args, set ) {
			...
			// when using a one-hard slider, wrap it in an array.
			if( !$.isArray(args) ){
				args = [args];
			}
			...
			return this.each(function(){
				...
				// mechanism twice for the first hard, to make sure it
				...
				for ( i = 0; i < handles.length; i++ ){
					...
					// Calculate a new position for the hard.
					to = args[ i%2 ];

					// The set request might want to ignore this hard.
					// Test for 'undefined' too, as a two-hard slider
					...
					// Calculate the new hard position
